...
| `admin.upload.fdiDesc` | Upload the Georgian FDI-by-country-by-sector XLSX file. The file must have a header row with "ქვეყნის კოდი" in column A, then one block per country (code, name, "სულ" row, sector sub-rows). Values must be in mln USD — the table shows them exactly as uploaded. | ატვირთეთ ქართული FDI-ქვეყანა-სექტორების XLSX ფაილი. ფაილს უნდა ჰქონდეს სათაურის ხაზი "ქვეყნის კოდი" A სვეტში, შემდეგ თითო ბლოკი ქვეყანაზე (კოდი, სახელი, "სულ" ხაზი, სექტორის ქვე-ხაზები). მნიშვნელობები უნდა იყოს მლნ. აშშ დოლარში — ცხრილი აჩვენებს მათ ზუსტად ისე, როგორც ატვირთულია. | States the mln-USD unit expectation |
